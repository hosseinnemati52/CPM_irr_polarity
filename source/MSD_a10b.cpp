...
void newMSD_a10b()
{
    ...
    int accepted_dt_base = 1e3;
    int accepted_dt;
    int m = 1;
    while (m < 10)
    {
        accepted_dt = m * accepted_dt_base;
        sampling_points.push_back( accepted_dt );
        m++;
    }
    
    accepted_dt_base = 1e4;
    m = 1;
    while (m < 10)
    {
        accepted_dt = m * accepted_dt_base;
    ...
    sampling_points.push_back( time[time.size()-1] - t_w );
    //////////////////// accepted dt //////////////////
    ...
    for (int dt_c = 0; dt_c < MSD_data_points_keys_vec.size(); dt_c++)
    {
        ...
        auto it = std::find(sampling_points.begin(), sampling_points.end(), dt);

        if (it == sampling_points.end()) 
        {continue;}
